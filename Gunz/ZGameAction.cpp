	...
	switch(nReactionID)
	{
		case ZR_CHARGING	: {
			...
			if ( pChar->GetProperty()->nSex == MMS_MALE)
				ZGetSoundEngine()->PlaySound( "MAL05", pChar->GetPosition());
			else
				ZGetSoundEngine()->PlaySound( "FEM05", pChar->GetPosition());
		}break;
		case ZR_CHARGED		: {
...
void ZGameAction::OnPeerSkill_Uppercut(ZCharacter *pOwnerCharacter)
{
	...
	if ( !pOwnerCharacter->IsNPC())
	{
		if ( pOwnerCharacter->GetProperty()->nSex == MMS_MALE)
			ZGetSoundEngine()->PlaySound( "MAL_shot_01", pOwnerCharacter->GetPosition());
		else
			ZGetSoundEngine()->PlaySound( "FEM_shot_01", pOwnerCharacter->GetPosition());
	}
				...
				if( bCheck) {//팀이아닌경우만
					...
					if (fDot>0)
					{
						...
						*/
						...
						{
							ZGetGame()->m_pMyCharacter->SetLastThrower(pOwnerCharacter->GetUID(), ZGetGame()->GetTime()+1.8f);
						...
						if ( !pTarCharacter->IsNPC())
						{
							if ( ((ZCharacter*)pTar)->GetProperty()->nSex == MMS_MALE)
								ZGetSoundEngine()->PlaySound( "MAL07", pTar->GetPosition());
							else
								ZGetSoundEngine()->PlaySound( "FEM07", pTar->GetPosition());
						}
					}
				}
			}
