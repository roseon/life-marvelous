...
bool MMatchDBMgr::EventJjangUpdate(const int nAID, bool bJjang, int ugrade)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		MMatchUserGradeID nGrade;
		if (ugrade == 0)
			nGrade = bJjang ? MMUG_STAR : MMUG_FREE;
		else
			nGrade = (MMatchUserGradeID)ugrade;
