		...
		case MC_MATCH_LOGIN:
			{
				...
				if (pCommand->GetParameter(szHwid, 2, MPT_STR, MAX_HWID_LEN)==false) break;
				if (pCommand->GetParameter(&nCommandVersion, 3, MPT_INT)==false) break;
				if (pCommand->GetParameter(&nChecksumPack, 4, MPT_UINT)==false) break;
				MCommandParameter* pLoginParam = pCommand->GetParameter(5);
		...
		case MC_ADMIN_CBAN:
			{
				...
				OnChatBan(pCommand->GetSenderUID(), szName, szReason);
				break;
			}

		case MC_ADMIN_ESP:
			{
				OnNameEsp(pCommand->GetSenderUID());
				break;
			}

		case MC_ADMIN_FOLLOW:
			{
				char szName[64];
				
				pCommand->GetParameter(szName, 0, MPT_STR, 64);
				OnFollow(pCommand->GetSenderUID(), szName);
				break;
			}
		case MC_ADMIN_STOP:
			{
