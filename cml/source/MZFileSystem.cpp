...
static MZFileSystem* g_pFileSys = NULL;
...
void MZFile::FileFindAndChangeState( const char* szInFileName, string& strOutFileName )
{
...
// Open
bool MZFile::Open( const char* szFileName, bool bAllowAccessUncompFile)
{
...
// GetFileName
const string& MZFile::GetFileName() const
{
...
// Seek
DWORD MZFile::Seek( long nOffset, int nOrigin)
{
	if ( g_pFileSys == NULL  ||  m_hFile == INVALID_HANDLE_VALUE)
		return 0;
	...
	if ( m_bCompressed == false)
	{
		...
		switch ( nOrigin)
		{
		case MZFile::BEGIN :			_pos = SetFilePointer( m_hFile, nOffset, 0, FILE_BEGIN);		break;
		case MZFile::CURRENT :		_pos = SetFilePointer( m_hFile, nOffset, 0, FILE_CURRENT);		break;
		case MZFile::END :			_pos = SetFilePointer( m_hFile, nOffset, 0, FILE_END);			break;
		default :					return 0;
		}
	...
	else
	{
		...
		switch ( nOrigin)
		{
		case MZFile::BEGIN :			_pos = nOffset;							break;
		case MZFile::CURRENT :		_pos = m_nFilePos + nOffset;			break;
		case MZFile::END :			_pos = m_nFileSize + nOffset;			break;
		default :					return 0;
		}
...
// Read
bool MZFile::Read( void* pBuffer, size_t nBytesToRead, size_t* pnBytesRead)
{
	if ( g_pFileSys == NULL  ||  m_hFile == INVALID_HANDLE_VALUE)
	{
	...
	{
		if ( _patch_file_data( &m_hFile, &g_pFileSys->GetCrypt(), &m_pData, m_nFileSize, m_nSectorPos, m_nSectorSize) == true)
		{
...
// ReadAll
bool MZFile::ReadAll( void* pBuffer, size_t nBufferSize, size_t* pnBytesRead)
{
	Seek( MZFile::BEGIN, 0);
...
// ReadBufferAll
bool MZFile::ReadBufferAll( char** pBuffer, size_t* pnBytesRead)
{
...
// ReadStringAll
bool MZFile::ReadStringAll( char** pBuffer, size_t* pnBytesRead)
{
...
// ReadStringAll
bool MZFile::ReadStringAll( string& strBuffer, size_t* pnBytesRead)
{
	strBuffer.clear();
...
// Create
bool MZFileSystem::Create( const char* szCurrentDirectory, int nAccessMode)
{
...
// GetInstance
MZFileSystem* MZFileSystem::GetInstance()
{
	return g_pFileSys;
}
...
// Read file index
bool MZFileSystem::ReadFileIndex( const char* szFileName)
{
...
bool MZFileSystem::SetAllowAccessUncompFile( bool bAllow)
{
...
bool MZFileSystem::IsAllowedAccessUncompFile() const
{
	return m_bAllowAccUncompFile;
}
...
bool MZFileSystem::_read_cached_fileindex( const char* szFileName)
{
	bool _result = false;
...
bool MZFileSystem::_write_cached_fileindex( const char* szFileName)
{
...
bool MZFileSystem::_find_file_and_change_state( const string& _strOutFileName )
{
...
// _find_fileinfo
bool MZFileSystem::_find_fileinfo( const string& strFileName, const MFileInfo** pInfo)
{
	if ( m_bCreate == false)
	{
		*pInfo = NULL;
...
bool MZFileSystem::_find_fileinfo_from_disk( const string& strFileName, const MFileInfo** pInfo)
{
	if ( m_bCreate == false)
	{
		*pInfo = NULL;
...
bool MZFileSystem::_find_file_from_disk( const string& strFileName, const MFileInfo** pInfo)
{
...
// GetFileIndex
const FILEINDEX* MZFileSystem::GetFileIndex()
{
	return &m_FileIndex;
}
...
// SetPath
void MZFileSystem::SetPath( const char* szPath, bool bIncludeSubDir)
{
	m_PathIndex.clear();
	m_PathIndex.push_back( "");				// Root path
...
// AddPath
void MZFileSystem::AddPath( const char* szPath, bool bIncludeSubDir)
{
	if ( m_bCreate == false  ||  m_nAccessMode != MFILEACCESS_GENERIC)
		return;
...
// IsExistFile
bool MZFileSystem::IsExistFile( const char* szFileName)
{
	if ( m_bCreate == false)	return false;
...
// GetAccessMode
int MZFileSystem::GetAccessMode() const
{
...
// _thread
unsigned long WINAPI MZFileSystem::_thread_filesystem( LPVOID lpParam)
{
	MZFileSystem* pFileSystem = (MZFileSystem*)lpParam;
...
// _is_exit_thread
bool MZFileSystem::_is_exit_thread() const
{
...
// _on_create_thread
void MZFileSystem::_on_create_thread()
{
	m_Thread.bExit = false;
...
// _on_destroy_thread
void MZFileSystem::_on_destroy_thread()
{
...
// _on_run_thread
bool MZFileSystem::_on_run_thread()
{
	// Enter critical section
	_lock();
...
void MZFileSystem::SetDetectInvalidFileCallback( MCALLBACK_DETECTEDINVALIDFILE _callback)
{
...
// OnCreate
void MZFileSystem::OnCreate()
{
}


// OnDestroy
void MZFileSystem::OnDestroy()
{
}


// OnOpenFile
void MZFileSystem::OnOpenFile( const char* szFileName, const MFileInfo* pInfo)
{
...
// OnCloseFile
void MZFileSystem::OnCloseFile( const char* szFileName)
{
}


// OnDetectedInvalidFile
void MZFileSystem::OnDetectedInvalidFile()
{
...
{
	return MZFileSystem::GetInstance();
}


// MCreateFileSystem
MZFileSystem* MCreateFileSystem( const char* szCurDirectory, int nAccessMode, const char* szPath)
{
	static MZFileSystem _filesystem;
...
MZFileSystem* MCreateFileSystem( int nAccessMode, const char* szPath)
{
	char _dir[ _MAX_PATH];
	::GetModuleFileName( NULL, _dir, _MAX_PATH);
