...
#define STR_ADDRESS_REAL					"localhost"//"gunz.abbygamerz.net"
...
void CNewAppDlg::StartGame()
{
	...
	if ( !bRunGameProcess && (curr_time > 1000))
	{
		...
		if ( m_bIsAlpha)
			bool a = cEncription.CreateSerialKey( 2);			// Alpha testing mode
		else
			bool b=  cEncription.CreateSerialKey( 0);
