...
#else 
				rs.Open("SELECT ServerID, Type, CurrPlayer, MaxPlayer, CONVERT(varchar(20), Time, 20) AS Time, IP, Port, ServerName, Opened FROM ServerStatus(NOLOCK) WHERE (Opened <> 0) AND (Opened IS NOT NULL)", 
					CRecordset::forwardOnly, CRecordset::readOnly);
#endif
			} else {
				// DB에서 가져올때 필터링해서 가져옴.
