...
void ZOptionInterface::InitInterfaceOption(void)
{
	...
	*/
	...
	{	
		...
		MEdit* pFPSEdit = (MEdit*)pResource->FindWidget( "FrameLimit_PerSecond" );
		if( pFPSEdit )
		{
			char szBuffer[1024];
			sprintf( szBuffer, "%d", Z_ETC_FRAMELIMIT_PERSECOND );
			pFPSEdit->SetText( szBuffer );
			pFPSEdit->SetMaxLength( 4 );
			RSetFrameLimitPerSeceond( Z_ETC_FRAMELIMIT_PERSECOND );
		}

		// 1ÃÊ´ç ÇÁ·¹ÀÓ Á¦ÇÑ
		//PenguinGuy - Commented this to make Frame Limiter not so shitty.
