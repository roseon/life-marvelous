...
#define EXT_LEN 4
	...
	int nTotalCount = 0;
	const FILEINDEX* index = pfs->GetFileIndex();
	auto itr = index->begin();
	for(int i=0; itr != index->end(); ++itr){
		const char* szFileName = (itr->first).c_str();
		const MFileInfo* desc = (itr->second);
		...
		for(int j=0;j<sizeof(loadExts)/sizeof(loadExts[0]);j++) {
			...
			if( nLen>EXT_LEN && stricmp(szFileName+nLen-EXT_LEN, loadExts[j])==0 )
			{
				...
				bool bMatch = false;
				// °æ·Î°¡ ¸Â¾Æ¾ßÇÑ´Ù
				if(nDirLen==0 || strnicmp(desc->strFileName.c_str(),szDir,nDirLen)==0)
					nTotalCount++;
				// custom crosshair
				if(strnicmp(desc->strFileName.c_str(),PATH_CUSTOM_CROSSHAIR,strlen(PATH_CUSTOM_CROSSHAIR))==0)
					nTotalCount++;
			}
		}
	} // °³¼ö¸¦ ¼¼±âÀ§ÇÑ Ä«ÇÇ
	...
	for(int i=0; itr != index->end(); ++itr){
		const char* pFileName = itr->first.c_str();
		char szFileName[256];
		...
		MFileInfo* desc = itr->second;
		int nLen = (int)strlen(szFileName);
		
		for(int j=0;j<sizeof(loadExts)/sizeof(loadExts[0]);j++) {
			// È®ÀåÀÚ°¡ ¸ÂÀ¸¸é..
			if( nLen>EXT_LEN && stricmp(strstr(szFileName, "."), loadExts[j])==0 )
			{
				bool bAddDirToAliasName = false;
				bool bMatch = false;
				// °æ·Î°¡ ¸Â¾Æ¾ßÇÑ´Ù
				...
				if(strnicmp(desc->strFileName.c_str(),PATH_CUSTOM_CROSSHAIR,strlen(PATH_CUSTOM_CROSSHAIR))==0)
				{
					...
					strcpy(szFileName, desc->strFileName.c_str());
				}
				...
				if(bMatch) {
					...
					else strcpy(aliasname, szFileName);
					...
					if(pBitmap->Create(aliasname, RGetDevice(), szFileName)==true)
...
//	SetListenerWidget("StagePlayerListTabFriend", ZGetListenerGamePlayerListTabFriend());
	...
	SetListenerWidget("Antilead", ZGetStageSettingChangedSettingListener());			// µ¿È¯ÀÌ°¡ Ãß°¡ : ³­ÀÔÇã¿ë
...
//	SetListenerWidget("MapList_Caller", ZGetStageMapListCallerListener());
	...
	*/
	...
	// ¼±ÅÃµÈ Ä³¸¯ÅÍ Ä«¸Þ¶ó·Î µ¹¸®¸ç º¸±â

	// ¸®½º³Ê ¾÷µ¥ÀÌÆ®µÈ ÅÇÄÜÆ®·Ñ °»½ÅÇØÁÖ±â.
...
//	ZApplication::GetSoundEngine()->CloseMusic();
	...
	/*switch ( nBitmap)
	{
		case ( 0) :
			...
			break;
		case ( 1) :
			strcpy( szFileName, "interface/loading_gaurd.jpg");
			break;
		case ( 2) :
			strcpy( szFileName, "interface/loading_ksa.jpg");
			break;
		case ( 3) :
			strcpy( szFileName, "interface/loading_safefall.jpg");
			break;
		case ( 4) :
			strcpy( szFileName, "interface/loading_tumbling.jpg");
			break;
		case ( 5) :
			strcpy( szFileName, "interface/loading_wallhang.jpg");
			break;
		case ( 6) :
			strcpy( szFileName, "interface/loading_walljump.jpg");
			break;
		case ( 7) :
			strcpy( szFileName, "interface/loading_wallrun01.jpg");
			break;
		case ( 8) :
			strcpy( szFileName, "interface/loading_wallrun02.jpg");
			break;
		default :
			strcpy( szFileName, "");
			break;
	...
	{
	case ( 0) :
		strcpy( szFileName, "interface/loading_1.jpg");
		break;
	case ( 1) :
		strcpy( szFileName, "interface/loading_2.jpg");
		break;
	case ( 2) :
...
//		MessageBox(NULL, ZMsg(MSG_HACKING_DETECTED), ZMsg( MSG_WARNING), MB_OK);
	...
	// ¾øÀ¸¸é ¿ø·¡ ÆÄÀÏÀ» ÀÐ´Â´Ù.
	if ( !bRead)
		bRead = m_pLoginBG->Create( "loginbg.png", RGetDevice(), "interface/loginbg.jpg");
	...
	((MBitmapR2*)m_pLoginPanel)->Create( "loginpanel.png", RGetDevice(), "interface/loginpanel.tga");
	if ( m_pLoginPanel)
	{
		// ÀÐ¾î¿Â ºñÆ®¸Ê ÀÌ¹ÌÁö Æ÷ÀÎÅÍ¸¦ ÇØ´ç À§Á¬¿¡ ³Ñ°ÜÁà¼­ Ç¥½ÃÇÑ´Ù
...
void ZGameInterface::OnDraw(MDrawContext *pDC)
{
	...
	// Ã»¼Ò³â ÀÚÀ² ±ÔÁ¦ Àû¿ë¾È(¾²¹ú¾²¹ú¾²¹ú...). 1½Ã°£¸¶´Ù ¸Þ½ÃÁö Á¶³½ ³¯¸®´Â°Å´Ù...
#ifdef LOCALE_KOREA			// ÇÑ±¹¿¡¼­¸¸ ÀÌÁþ°Å¸® ÇÑ´ç...
...
void ZGameInterface::ChangeWeapon(ZChangeWeaponType nType)
{
	...
	if (pChar->IsDie() ) return;						// Á×¾úÀ¸¸é ¹«½ÃÇÑ´Ù.	
	if (m_pGame->GetMatch()->IsRuleGladiator()) return;	// ±Û·¡µð¿¡ÀÌÅÍ¸é Ä®¸¸½á¶ó


	int nParts = -1;

/*
	if( m_bisReserveChangeWeapon ) { // ¿¹¾àµÈ»óÅÂ¿¡¼­ ¹Ù²Ù·Á°í ÇÏ´Â°æ¿ì ¿¹¾à Ãë¼Ò~
		m_bisReserveChangeWeapon = false;
	}

	if( m_pMyCharacter->m_fNextShotTime > g_pGame->GetTime()) { // ¾ÆÁ÷ ¹Ù²Ü¼öÀÖ´Â½Ã°£ÀÌ ¾Æ´Ï µÇ¾úÀ¸´Ï ¿¹¾à~
...
*/
	...
	bool bWheel = false;		// ÈÙ·Î ÀÛµ¿ÇßÀ»¶§´Â Äµ½½µÇÁö ¾Ê´Â´Ù
	...
	// ¸Å´Þ·ÁÀÖÀ»¶§/½ò¶§/´ýºí¸µ/½ºÅ³/¸·À»¶§/¶ç¿öÁø»óÅÂ´Â ¹«±â±³È¯ÀÌ ¾ÈµÈ´Ù.
	if (zStatus.m_bWallHang || zStatus.m_bShot || zStatus.m_bShotReturn || uStatus.m_bTumble 
		...
		|| zStatus.m_bSlash || zStatus.m_bJumpSlash || zStatus.m_bJumpSlashLanding	// °­º£±â Áß¿¡µµ ±³È¯ ¾ÈµÈ´Ù
		 
		// ¼¦ÁßÀÏ¶§ »óÃ¼¾Ö´Ï¸ÞÀÌ¼ÇÀ¸·Î ÆÇº°
		|| (pChar->GetStateUpper() == ZC_STATE_UPPER_SHOT && pChar->IsUpperPlayDone() == false )
		
		/*
		// ±×¸®°í ¹ß»çÇÏ°í³ª¼­ µô·¹ÀÌ°¡ ¾ÆÁ÷ ¾È³¡³µÀ»¶§
	...
	/*
	// »óÃ¼¾Ö´Ï¸ÞÀÌ¼ÇÀÌ ÀÖÀ¸¸é Äµ½½.. ¸®·Îµå/¼¦ µûÀ§
	...
	*/
	...
	// ³» ¹«±â´Â Á÷Á¢ ¹Ù²Û´Ù. command¸¦ ÅëÇÏ¸é ¼ø¼­°¡ ¹Ù²î´Â °æ¿ì°¡ ÀÖ´Ù.
...
void ZGameInterface::OnCharSelectionCreate(void)
{
	...
	if( ZApplication::GetSoundEngine()->HasMusicStopped() )
	{
		ZApplication::GetSoundEngine()->OpenCustomMusicFileByRand();
