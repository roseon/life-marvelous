...
bool ZSoundEngine::OpenCustomMusic( const char* szFileNameDir, const char* szFileName )
{
	...
	if( m_szOpenedMusicName[0] != 0 )
		CloseMusic();

	strcpy( m_szOpenedMusicName, szFileName );

	int nLen = 0;
	FILE* pFile = fopen( szFileNameDir, "rb" );
	if( !pFile )
		return false;
...
//PenguinGuy
bool ZSoundEngine::OpenCustomMusicFileByRand()
{
	char szTemp[MAX_PATH];

	int nRandomNumber = RandomNumber( 0.0f, (float)m_nNumOfMusics );
	sprintf( szTemp, "%s\\%s", m_szMusicPath, m_szCustomMusicNames[nRandomNumber] );

	m_nLastSongIndex = nRandomNumber;

	if( OpenCustomMusic( szTemp, m_szCustomMusicNames[nRandomNumber] ) )
		return true;
...
void ZSoundEngine::MusicEndCallback(void* pCallbackContext)
{
	...
	pSoundEngine->OpenCustomMusicFileByRand();
...
void ZSoundEngine::PlaySoundChargeComplete(MMatchItemDesc *pDesc, const rvector& pos)
{
	if(strcmp("rubber", pDesc->m_szWeaponByFiber) == 0)
	{
		PlaySound("FX_rubber_ChargeComplete", pos );	// 고무로 벽긁는 소리(플런쳐X:502009)
		return;
	}

	PlaySound("FX_ChargeComplete", pos );	// 디폴트 쇠로 벽긁는 소리
}
