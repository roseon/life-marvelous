...
void ZCharacter::OnUpdate(float fDelta)
{
	if (m_bInitialized==false) return;
	if (!IsVisible()) return;
	if (m_Status.Ref().nFrozen) return;
...
*/
	...
	if ( uStatus.m_bFallingToNarak == true)
	{
		if (m_Property.nSex == MMS_MALE)
			ZGetSoundEngine()->PlaySound("MAL10",GetPosition(),IsObserverTarget());
		else
			ZGetSoundEngine()->PlaySound("FEM10",GetPosition(),IsObserverTarget());
	}
	else
	{
		if (m_Property.nSex == MMS_MALE)
			ZGetSoundEngine()->PlaySound("MAL08",GetPosition(),IsObserverTarget());
		else
			ZGetSoundEngine()->PlaySound("FEM08",GetPosition(),IsObserverTarget());
	}
...
//					int k=0;
	...
	if ( uStatus.m_bDamaged && (!IsDie()) && (GetHP() < 30.f))
	{
		...
		if ( (m_damageInfo.Ref().m_dwLastDamagedTime + 5000) < currTime)
		{
			...
			{
				int nRetVal = ZGetSoundEngine()->PlaySound("MAL06", GetPosition(), IsObserverTarget());
				if ( nRetVal == 0)
			...
			{
 				int nRetVal = ZGetSoundEngine()->PlaySound("FEM06", GetPosition(), IsObserverTarget());
				if ( nRetVal == 0)
...
void ZCharacter::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	if(!bCanAttack)
		PROTECT_DEBUG_REGISTER(!bDebugRegister)
			...
			{ 
				GetStatus().CheckCrc();
				GetStatus().Ref().nDamageTaken += fDamage; 
				GetStatus().MakeCrc();
			}
