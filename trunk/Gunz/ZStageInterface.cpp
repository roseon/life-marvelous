...
void ZStageInterface::OnCreate( void)
{
	...
	if ( pPicture)
	{
		...
		((MBitmapR2*)m_pStageFrameImg)->Create( "stage_frame.png", RGetDevice(), "interface/stage_frame.png");
	...
	if ( pPicture)
	{
		...
		((MBitmapR2*)m_pItemListFrameImg)->Create( "itemlistframe.tga", RGetDevice(), "interface/itemlistframe.tga");
	...
	{ // 릴레이맵 "RelayMap" 초기화
		...
		{ // f릴레이맵 리스트 배경 이미지
			...
			((MBitmapR2*)m_pRelayMapListFrameImg)->Create( "relaymaplistframe.tga", RGetDevice(), "interface/relaymaplistframe.tga");
...
void ZStageInterface::ChangeStageGameSetting( const MSTAGE_SETTING_NODE* pSetting)
{
	...
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_SOLO) ||			// 데쓰매치 개인전이거나...
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_DEITY ) ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_DUEL))							// 듀얼모드 이면...
	{
		...
		(pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM2) ||			// 무한데스매치 팀전이거나...
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_AWP)  ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_GRENADEWAR)  ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_SKILLWAR)  ||
